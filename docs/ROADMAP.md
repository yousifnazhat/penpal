...
Status snapshot, last updated 2026-07-12:
...
| PI cockpit | Cross-platform prerelease-ready | Pinned PI bootstrap and offline RPC smoke pass on Linux and Windows; seven read-only tools and `/penpal-status` are verified | Keep the repo-local package supported; verify a scoped npm name before publication |
...
| Public v1 | Integrated prerelease ready | Python wheel/sdist install smoke, cross-platform core CI, and bundled PI CI are green for `v0.2.0-rc.1` | Collect onboarding friction and verify registry ownership |
